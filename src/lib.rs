...
/// Selects the language/extension of an in-memory entry source (see
/// [`run_source`]).
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum SourceLang {
    /// The source is JavaScript — the virtual entry gets a `.js` extension
    /// (MediaType::JavaScript).
    JavaScript,
    /// The source is TypeScript — the virtual entry gets a `.ts` extension
    /// (MediaType::TypeScript) and is transpiled by the normal graph pipeline.
    TypeScript,
}
...
/// The entry of a run: a real filesystem path, or an in-memory source backed
/// by a virtual `file:` URL registered in the run's per-run
/// `RuntimeServices::memory_files` map (see [`run_source`]).
#[derive(Clone)]
...
pub(crate) fn entry_source_memory(
    ...
    let base = std::fs::canonicalize(&base).map_err(|e| {
        LibdenoError::Configuration(format!(
            "in-memory base_dir does not exist or cannot be resolved: {}: {e}",
            base.display()
        ))
    })?;
    if !base.is_dir() {
        return Err(LibdenoError::Configuration(format!(
            "in-memory base_dir is not a directory: {}",
            base.display()
        )));
    }
    Ok(EntrySource::Memory {
...
/// Runs JS/TS **source code held in memory** (`&str`, `String`, `&[u8]`,
...
///
/// - Relative imports resolve against `base_dir` (the virtual file's parent),
///   so `import "./sibling.js"` loads `<base_dir>/sibling.js` from disk.
/// - `import.meta.url` / `Deno.mainModule` show the virtual specifier, not a
///   caller-chosen name; the URL is unique per call and stable for the
///   duration of that run.
/// - The entry itself is not read from disk, but its virtual path is still
///   subject to the entry read permission check (against `base_dir`, which
///   is always inside the virtual path's canonical parent); every *child*
///   import is checked against `options.permissions` as usual. Granting read
///   on `base_dir` covers both the virtual entry path and its on-disk
///   sibling imports.
...
///
/// Semantics match [`run`] in every other respect: each call builds its own
/// current-thread runtime and worker; ordinary runs execute in parallel;
/// output capture is exclusive (see [`run_with_output`]); tokio re-entry is
/// handled by running on a fresh thread when called from inside a tokio
/// runtime. The source bytes are kept alive (`Arc<[u8]>`) for the run's
/// lifetime; mutating the original buffer afterwards does not affect the run.
pub fn run_source(
    code: impl AsRef<[u8]>,
    lang: SourceLang,
    base_dir: impl AsRef<Path>,
    ...
    run_source_with_output(code, lang, base_dir, options).map(|o| o.exit_code)
}

/// Captured-output variant of [`run_source`]: same semantics as
/// [`run_source`] — including the tokio re-entry escape (runs on a fresh
/// thread inside a tokio context) — with the capture-exclusivity lease
/// semantics of [`run_with_output`] (see `RunLease`).
...
/// Async variant of [`run_source`] on the caller's tokio runtime: same
/// semantics as [`run_async`] — the future is `!Send` and must not be
/// interleaved with another async run on the same thread (see
/// [`run_async`]). Memory entries inherit exactly the same tokio re-entry /
/// capture-lease behavior as path entries (the shared guard + lease helpers).
pub async fn run_source_async(
    ...
    run_source_with_output_async(code, lang, base_dir, options)
        .await
        .map(|o| o.exit_code)
}

/// Captured-output variant of [`run_source_async`]; see
/// [`run_with_output_async`] for the runtime-context and capture semantics.
...
pub(crate) async fn run_inner_with_cancellation(
    ...
    let (permissions, services, main_module) = {
        ...
        // RuntimeServices::new builds the per-run memory-files map; resolving
        // the main module afterwards registers an in-memory entry into exactly
        // that map (the shared stack outlives runs, so keeping virtual files
        // there would leak per run).
...
/// Resolve the run's main module from an [`EntrySource`]. For a path entry
/// this is [`resolve_entry`] unchanged. For an in-memory entry it builds a
/// unique virtual `file:` URL under the entry's (canonicalized) base
/// directory and registers the source in the run's per-run
...
async fn resolve_main_module(
    ...
    entry: &EntrySource,
    cwd: &Path,
) -> Result<ModuleSpecifier, LibdenoError> {
    match entry {
        EntrySource::Path(path) => resolve_entry(path, cwd).map_err(LibdenoError::Entry),
        EntrySource::Memory {
            code,
            ext,
            base_dir,
            ...
            memory_files.insert(
                ...
                deno_cache_dir::file_fetcher::File {
                    ...
                    maybe_headers: None,
                    source: code.clone(),
                    loaded_from: deno_cache_dir::file_fetcher::LoadedFrom::Unknown,
                },
            );
            Ok(url)
        }
